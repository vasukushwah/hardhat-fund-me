...
describe("Fundme", async () => {
    ...
    describe("fund", async () => {
        ...
        it("Should be update funded amounts", async () => {
            ...
            const response = await fundMe.s_addressToFundedAmount(deployer)
    ...
    describe("withdraw", async () => {
        ...
        it("Chiper Withdraw Eth from a single founder", async () => {
            const startingFundMeBalance = await fundMe.provider.getBalance(
                fundMe.address
            )
            const startingDeployerBalance = await fundMe.provider.getBalance(
        ...
        it("Allow us to withdraw with multiple funders", async () => {
            ...
            for (let i = 1; i < 6; i++) {
                assert.equal(
                    await fundMe.s_addressToFundedAmount(accounts[i].address),
                    0
                )
            }
        })
